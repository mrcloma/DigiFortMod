<#
.SYNOPSIS
    Funcao do modulo Empresa para consultar o MachineCode de um servidor.
...
.PARAMETER Properties
    E necessario indicar o parametro -Properties para que a listagem dos machine code venha com apenas as Properties indicadas. As Properties disponíveis são MachineCode e ServerID
	
.EXAMPLE
    Get-MachineCode -Server monitoramento4
...
function Get-MachineCode {
	...
	param (
		...
		[string]$Server,
		[string[]]$Properties = @("MachineCode", "ServerID"))
    ...
    $apiUrl = "http://${Server}:8601/Interface/Server/GetMachineCode?AuthSession=$AuthSessionID&AuthData=$AuthData&ResponseFormat=JSON"
    ...
    try {
        ...
        if ($jsonData.Response) {
            $filteredData = $jsonData.Response.Data.MachineCode | Select-Object -Property $Properties
            $filteredData
        } else {
...
Export-ModuleMember -Function 'Get-MachineCode'
